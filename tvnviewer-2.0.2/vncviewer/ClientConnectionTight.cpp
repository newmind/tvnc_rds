...
void ClientConnection::ReadTightRect(rfbFramebufferUpdateRectHeader *pfburh)
{
    ...
    if ((comp_ctl & 1) && m_tightZlibStreamActive[i]) {
      ...
      if (err != Z_OK) {
        if (m_tightZlibStream[i].msg != NULL) {
          Log::interror(_T("zlib inflateEnd() error: %s\n"),
                       m_tightZlibStream[i].msg);
        } else {
          Log::interror(_T("zlib inflateEnd() error: %d\n"), err);
        }
        return;
      }
  ...
  if (comp_ctl >= rfbTightMaxSubencoding) {
    Log::interror(_T("Tight encoding: bad subencoding value received.\n"));
    return;
  }
  ...
  if ((comp_ctl & rfbTightExplicitFilter) != 0) {
    ...
    case rfbTightFilterGradient:
      ...
      break;
    default:
      Log::interror(_T("Tight encoding: unknown filter code received.\n"));
      return;
    }
  } else {
    bitsPixel = InitFilterCopy(pfburh->r.w, pfburh->r.h);
  ...
  if (bitsPixel == 0) {
    Log::interror(_T("Tight encoding: error receiving palette.\n"));
    return;
  }
  ...
  if (compressedLen <= 0) {
    Log::interror(_T("Tight encoding: bad data received from server.\n"));
    return;
  }
  ...
  if (!m_tightZlibStreamActive[stream_id]) {
    ...
    if (err != Z_OK) {
      if (zs->msg != NULL) {
        Log::interror(_T("zlib inflateInit() error: %s.\n"), zs->msg);
      } else {
  ...
  while (compressedLen > 0) {
    ...
    do {
      ...
      if (err != Z_OK && err != Z_STREAM_END) {
        if (zs->msg != NULL) {
          Log::interror(_T("zlib inflate() error: %s.\n"), zs->msg);
        } else {
  ...
  if (rowsProcessed != pfburh->r.h) {
    Log::interror(_T("Tight encoding: wrong number of scan lines.\n"));
  }
}
...
void ClientConnection::DecompressJpegRect(int x, int y, int w, int h)
{
  ...
  if (compressedLen <= 0) {
    Log::interror(_T("Incorrect data received from the server.\n"));
    return;
  }
  ...
  if ((int)cinfo.output_width != w || (int)cinfo.output_height != h ||
    ...
    Log::interror(_T("Tight Encoding: Wrong JPEG data received.\n"));
